...
class JoinFeatures(object):
    ...
    def description(self) -> str:
        ...
        JoinFeatures_geoanalytics(target_layer, join_layer, output_name, join_operation, {spatial_relationship}, {spatial_near_distance}, {temporal_relationship}, {temporal_near_distance}, {attribute_relationship;attribute_relationship...}, {summary_fields;summary_fields...}, {join_condition}, {data_store}, {keep_all_target_features}, {include_distance}, {distance_unit})
     ...
     INPUTS:
      ...
      spatial_relationship {String}:
          ...
          Enterprise 10.7 or later.CONTAINS-The features in the join features
          will be matched if a target
      ...
      temporal_relationship {String}:
          ...
          10.6 or later.NEAR_AFTER-When a target time is after the join time but
          within a
          specified range of time from the join time, the target time is near
      ...
      join_condition {String}:
          ...
          the Income attribute in the target layer. In ArcGIS Enterprise 10.5
          and 10.5.1, the join condition to use to apply this expression is
          join["HealthSpending"] > target["Income"] * .2. In ArcGIS Enterprise
          10.6 or later, use an Arcade expression such as
          $join["HealthSpending"] > $target["Income"] * .2.
      data_store {String}:
